...
Dependencies: `requirements.txt` in this directory, pinned to the versions that
produced the committed results. The repository-root `../requirements.txt` covers
every work-stream in the workspace and pins slightly different versions; prefer
the local file when reproducing this paper.

Clinical data is reached through `chat1_protocol/data`, a Windows directory
junction into the shared dataset store at `RES/datasets` (which supplies
`physionet2019/`, `mimic4-demo/` and `eICU-demo/`). The junction is not tracked
by git. Recreate it with:

    New-Item -ItemType Junction -Path <repo>/chat1_protocol/data -Target <path to RES/datasets>

or point `PHYSIONET_DIR`, `MIMIC_DIR` and `EICU_DIR` straight at your own copies
— `config.py` reads all three from the environment. Without either, the detector
5 sampling tests SKIP with `PhysioNet data not present` and `run_all.py` still
reproduces the full table from the committed aggregates.

**Never delete that junction with `rm -rf`** — it can follow the link and destroy
the shared dataset store that every other project under `RES/` also uses. Remove
the link alone, with `cmd /c rmdir <path>` or `Remove-Item <path>` (no
`-Recurse`).
...
Verified on 2026-09-06 by cloning the repository into an empty directory with
**no clinical data present at all** and running:

    pip install -r requirements.txt
    cd chat1_protocol
    PCL_TEST_MODE=1 python detectors/run_all.py

That prints the table below, `TOTAL TP=6 FP=0 FN=1 TN=12`, on a machine with no
PhysioNet, MIMIC-IV or eICU access. Checks 3 and 4 are static analysis and run
live; checks 1, 2 and 5 load the committed aggregate results in
`detectors/results/*.json`.

The test suite behaves the same way:

    PCL_TEST_MODE=1 python -m unittest discover -s detectors/tests -t . -q

116 tests, 7 skipped with the reason `PhysioNet data not present`. Those seven
are detector 5's sampling tests, and they SKIP rather than pass — a distinction
this paper argues for at length, and one worth checking in your own diagnostics.

Per-stay caches are excluded from the repository; see `CONTRIBUTORS.md` and
`detectors/TODO.md` section 0 to regenerate them from raw data with your own
credentials. Nothing above needs them.

`TAXONOMY.md` names the four failure modes a diagnostic can have independently
of whether its target confound is real. Every finding below is an instance of
one of them, and the paper's framing depends on that file rather than on prose
here.
